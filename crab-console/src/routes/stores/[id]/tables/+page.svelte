<script lang="ts">
	...
	import { ArrowLeft, Grid3x3, Plus, Pencil, Trash2, X } from 'lucide-svelte';
	...
	import {
		...
		deleteTable,
		ApiError,
		type CatalogDiningTable,
		type CatalogZone,
		type DiningTableCreate,
		type DiningTableUpdate
	} from '$lib/api';
	import ConsoleLayout from '$lib/components/ConsoleLayout.svelte';

	const storeId = Number(page.params.id);

	let tables = $state<CatalogDiningTable[]>([]);
	...
	let editing = $state<CatalogDiningTable | null>(null);
	let saving = $state(false);
	let modalError = $state('');

	let form = $state({ name: '', zone_id: 0, capacity: 4 });

	// Delete confirm
	let deleting = $state<CatalogDiningTable | null>(null);
	let deleteLoading = $state(false);

	function zoneName(zoneId: number): string {
		return zones.find((z) => z.id === zoneId)?.name ?? $t('tables.no_zone');
	}

	function openCreate() {
		editing = null;
		form = { name: '', zone_id: zones[0]?.id ?? 0, capacity: 4 };
		modalError = '';
		showModal = true;
	}

	function openEdit(table: CatalogDiningTable) {
		editing = table;
		form = { name: table.name, zone_id: table.zone_id, capacity: table.capacity };
		modalError = '';
		showModal = true;
	}
	...
	async function handleSave() {
		...
		try {
			if (editing) {
				const data: DiningTableUpdate = {
					name: form.name,
					zone_id: form.zone_id,
					capacity: form.capacity
				};
				const res = await updateTable(token, storeId, editing.id, data);
				if (!res.success) { modalError = res.error ?? $t('catalog.error'); saving = false; return; }
			} else {
				const data: DiningTableCreate = {
					name: form.name,
					zone_id: form.zone_id,
					capacity: form.capacity
				};
				const res = await createTable(token, storeId, data);
				if (!res.success) { modalError = res.error ?? $t('catalog.error'); saving = false; return; }
			}
			showModal = false;
			await reload();
	...
	async function handleDelete() {
		...
		deleteLoading = true;
		try {
			const res = await deleteTable(token, storeId, deleting.id);
	...
	async function reload() {
		try {
			[tables, zones] = await Promise.all([getTables(token, storeId), getZones(token, storeId)]);
		} catch (err) {
...
<svelte:head>
	<title>{$t('tables.title')} — RedCoral Console</title>
</svelte:head>
	...
	<div class="max-w-3xl mx-auto px-4 py-4 md:px-6 md:py-8 space-y-4 md:space-y-6">
		<div class="flex items-center justify-between">
			<div class="flex items-center gap-3">
				...
				<h1 class="font-heading text-lg md:text-xl font-bold text-slate-900">{$t('tables.title')}</h1>
			</div>
			<button
				onclick={openCreate}
		...
		{:else if tables.length === 0}
			<div class="bg-white rounded-2xl border border-slate-200 p-8 text-center">
				...
				<p class="text-sm text-slate-500">{$t('tables.empty')}</p>
			</div>
		{:else}
				...
				{#each tables as table}
					<div class="flex items-center justify-between px-5 py-3.5">
						<div class="flex items-center gap-3 min-w-0">
							<span class="text-sm font-medium {table.is_active ? 'text-slate-900' : 'text-slate-400 line-through'}">{table.name}</span>
							<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-indigo-50 text-indigo-500">{zoneName(table.zone_id)}</span>
							<span class="text-xs text-slate-400">{$t('tables.capacity')}: {table.capacity}</span>
							{#if !table.is_active}
								<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-slate-100 text-slate-400">{$t('catalog.inactive')}</span>
							{/if}
						</div>
						<div class="flex items-center gap-1 shrink-0 ml-4">
							<button onclick={() => openEdit(table)} class="p-1.5 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-slate-50 cursor-pointer">
								<Pencil class="w-3.5 h-3.5" />
							</button>
							<button onclick={() => (deleting = table)} class="p-1.5 text-slate-400 hover:text-red-500 rounded-lg hover:bg-red-50 cursor-pointer">
	...
	<div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onclick={() => (showModal = false)} role="dialog">
		<div class="bg-white rounded-2xl shadow-xl w-full max-w-md" onclick={(e) => e.stopPropagation()}>
			<div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
				<h2 class="font-heading font-semibold text-slate-900">{editing ? $t('tables.edit') : $t('tables.new')}</h2>
			...
			<div class="p-6 space-y-4">
				<div class="space-y-1">
					<label for="table-name" class="block text-sm font-medium text-slate-700">{$t('catalog.name')}</label>
					<input type="text" id="table-name" bind:value={form.name}
						class="w-full rounded-lg border border-slate-300 px-3 py-2 focus:border-coral-500 focus:ring-coral-500 text-sm" />
				</div>
				<div class="space-y-1">
					<label for="table-zone" class="block text-sm font-medium text-slate-700">{$t('tables.zone')}</label>
					<select id="table-zone" bind:value={form.zone_id}
						class="w-full rounded-lg border border-slate-300 px-3 py-2 focus:border-coral-500 focus:ring-coral-500 text-sm">
						{#each zones as zone}
				...
				<div class="space-y-1">
					<label for="table-cap" class="block text-sm font-medium text-slate-700">{$t('tables.capacity')}</label>
					<input type="number" id="table-cap" bind:value={form.capacity} min="1"
						class="w-full rounded-lg border border-slate-300 px-3 py-2 focus:border-coral-500 focus:ring-coral-500 text-sm" />
				</div>
				{#if modalError}
