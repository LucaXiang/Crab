<script lang="ts">
	...
	import { ArrowLeft, FolderTree, Plus, Pencil, Trash2, X } from 'lucide-svelte';
	...
	let editing = $state<CatalogCategory | null>(null);
	...
	let deleting = $state<CatalogCategory | null>(null);
	let deleteLoading = $state(false);

	function openCreate() {
		editing = null;
		form = { name: '', sort_order: 0, is_virtual: false, is_display: true };
		modalError = '';
		showModal = true;
	}
	...
	function openEdit(cat: CatalogCategory) {
		...
		form = {
			...
			is_virtual: cat.is_virtual,
			is_display: cat.is_display
		};
		modalError = '';
		showModal = true;
	}
	...
	async function handleSave() {
		...
		try {
			if (editing) {
				...
				const res = await updateCategory(token, storeId, editing.source_id, data);
				...
				const res = await createCategory(token, storeId, data);
				if (!res.success) { modalError = res.error ?? $t('catalog.error'); saving = false; return; }
			}
			showModal = false;
			await reload();
	...
	async function handleDelete() {
		...
		deleteLoading = true;
		try {
			const res = await deleteCategory(token, storeId, deleting.source_id);
	...
	async function reload() {
		try {
			categories = await getCategories(token, storeId);
		} catch (err) {
...
<svelte:head>
	<title>{$t('categories.title')} — RedCoral Console</title>
</svelte:head>
	...
	<div class="max-w-3xl mx-auto px-4 py-4 md:px-6 md:py-8 space-y-4 md:space-y-6">
		<div class="flex items-center justify-between">
			<div class="flex items-center gap-3">
				...
				<h1 class="font-heading text-lg md:text-xl font-bold text-slate-900">{$t('categories.title')}</h1>
			</div>
			<button
				onclick={openCreate}
		...
		{:else if categories.length === 0}
			<div class="bg-white rounded-2xl border border-slate-200 p-8 text-center">
				<FolderTree class="w-10 h-10 text-slate-300 mx-auto mb-3" />
				<p class="text-sm text-slate-500">{$t('categories.empty')}</p>
			</div>
		{:else}
				...
				{#each categories as cat}
					<div class="flex items-center justify-between px-5 py-3.5">
						<div class="flex items-center gap-3 min-w-0">
							<div class="w-9 h-9 rounded-lg flex items-center justify-center shrink-0 {cat.is_active ? 'bg-coral-50' : 'bg-slate-50'}">
								<FolderTree class="w-4 h-4 {cat.is_active ? 'text-coral-500' : 'text-slate-300'}" />
							</div>
							<div class="min-w-0">
								<p class="text-sm font-medium {cat.is_active ? 'text-slate-900' : 'text-slate-400 line-through'}">{cat.name}</p>
								<div class="flex items-center gap-2 text-xs text-slate-400">
									{#if cat.is_virtual}
										<span class="px-1.5 py-0.5 rounded-full bg-purple-50 text-purple-500">{$t('categories.virtual')}</span>
									{/if}
									{#if !cat.is_display}
										<span class="px-1.5 py-0.5 rounded-full bg-slate-100 text-slate-400">hidden</span>
									{/if}
									{#if !cat.is_active}
						...
						<div class="flex items-center gap-1 shrink-0 ml-4">
							<button onclick={() => openEdit(cat)} class="p-1.5 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-slate-50 cursor-pointer">
								<Pencil class="w-3.5 h-3.5" />
							</button>
							<button onclick={() => (deleting = cat)} class="p-1.5 text-slate-400 hover:text-red-500 rounded-lg hover:bg-red-50 cursor-pointer">
	...
	<div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onclick={() => (showModal = false)} role="dialog">
		<div class="bg-white rounded-2xl shadow-xl w-full max-w-md" onclick={(e) => e.stopPropagation()}>
			<div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
				<h2 class="font-heading font-semibold text-slate-900">{editing ? $t('categories.edit') : $t('categories.new')}</h2>
			...
			<div class="p-6 space-y-4">
				<div class="space-y-1">
					<label for="cat-name" class="block text-sm font-medium text-slate-700">{$t('catalog.name')}</label>
					<input type="text" id="cat-name" bind:value={form.name}
						class="w-full rounded-lg border border-slate-300 px-3 py-2 focus:border-coral-500 focus:ring-coral-500 text-sm" />
				</div>
				<div class="space-y-1">
					<label for="cat-order" class="block text-sm font-medium text-slate-700">{$t('catalog.sort_order')}</label>
					<input type="number" id="cat-order" bind:value={form.sort_order}
						class="w-full rounded-lg border border-slate-300 px-3 py-2 focus:border-coral-500 focus:ring-coral-500 text-sm" />
				</div>
				<div class="flex items-center gap-6">
					<label class="flex items-center gap-2 cursor-pointer">
						<input type="checkbox" bind:checked={form.is_virtual} class="rounded border-slate-300 text-coral-500 focus:ring-coral-500" />
						<span class="text-sm text-slate-700">{$t('categories.virtual')}</span>
					</label>
					<label class="flex items-center gap-2 cursor-pointer">
						<input type="checkbox" bind:checked={form.is_display} class="rounded border-slate-300 text-coral-500 focus:ring-coral-500" />
						<span class="text-sm text-slate-700">{$t('categories.display')}</span>
					</label>
				</div>
				{#if modalError}
