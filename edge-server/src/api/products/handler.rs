...
pub async fn create(
    ...
    // 广播同步通知
    let id = product.id.as_ref().map(|t| t.id.to_string());
    state.broadcast_sync(RESOURCE_PRODUCT, id.as_deref(), "created", Some(&product)).await;
...
pub async fn update(
    ...
    // 广播同步通知
    state.broadcast_sync(RESOURCE_PRODUCT, Some(&id), "updated", Some(&product)).await;
...
pub async fn delete(
    ...
    // 广播同步通知
    if result {
        state.broadcast_sync::<()>(RESOURCE_PRODUCT, Some(&id), "deleted", None).await;
    }
...
pub async fn create_spec(
    ...
    // 广播同步通知
    let id = spec.id.as_ref().map(|t| t.id.to_string());
    state.broadcast_sync(RESOURCE_SPEC, id.as_deref(), "created", Some(&spec)).await;
...
pub async fn update_spec(
    ...
    // 广播同步通知
    state.broadcast_sync(RESOURCE_SPEC, Some(&id), "updated", Some(&spec)).await;
...
pub async fn delete_spec(
    ...
    // 广播同步通知
    if result {
        state.broadcast_sync::<()>(RESOURCE_SPEC, Some(&id), "deleted", None).await;
    }
...
pub async fn add_tag(
    ...
    // 广播同步通知
    state.broadcast_sync(RESOURCE_SPEC, Some(&spec_id), "updated", Some(&spec)).await;
