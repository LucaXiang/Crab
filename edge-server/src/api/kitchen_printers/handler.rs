...
pub async fn create(
    ...
    // 广播同步通知
    let id = printer.id.as_ref().map(|t| t.id.to_string());
    state.broadcast_sync(RESOURCE, id.as_deref(), "created", Some(&printer)).await;
...
pub async fn update(
    ...
    // 广播同步通知
    state.broadcast_sync(RESOURCE, Some(&id), "updated", Some(&printer)).await;
