...
use crate::db::models::{PrintDestinationCreate, PrintDestinationUpdate};
use crate::db::repository::PrintDestinationRepository;
use crate::utils::{AppError, AppResult};
use shared::models::PrintDestination as SharedPrintDestination;
...
pub async fn list(
    ...
    Ok(Json(items.into_iter().map(|p| p.into()).collect()))
}
...
pub async fn get_by_id(
    State(state): State<ServerState>,
    Path(id): Path<String>,
) -> AppResult<Json<SharedPrintDestination>> {
    let repo = PrintDestinationRepository::new(state.db.clone());
    let item = repo
        .find_by_id(&id)
        .await
