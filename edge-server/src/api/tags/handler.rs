...
pub async fn list(
    ...
    let tags = repo.find_all().await.map_err(|e| AppError::database(e.to_string()))?;
    Ok(Json(tags))
}
...
pub async fn create(
    ...
    let repo = TagRepository::new(state.db.clone());
    let tag = repo.create(payload).await.map_err(|e| AppError::database(e.to_string()))?;

    // 广播同步通知
    let id = tag.id.as_ref().map(|t| t.id.to_string());
    state.broadcast_sync(RESOURCE, id.as_deref(), "created", Some(&tag)).await;
...
pub async fn update(
    ...
    // 广播同步通知
    state.broadcast_sync(RESOURCE, Some(&id), "updated", Some(&tag)).await;
