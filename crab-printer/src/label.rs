...
use windows::Win32::Graphics::Gdi::{
    ...
    DM_PAPERLENGTH, DM_PAPERSIZE, DM_PAPERWIDTH, DMBIN_FORMSOURCE, DMPAPER_USER, DeleteDC,
    GetDeviceCaps, HDC, LOGPIXELSX, LOGPIXELSY, SRCCOPY, StretchDIBits,
};
...
use windows::core::{Error, HRESULT, PCWSTR, PWSTR, Result as WinResult, w};
...
pub fn print_rgba_premul(
    ...
    let printer = crate::WindowsPrinter::resolve(options.printer_name.as_deref())
        .map_err(|e| Error::new(E_INVALIDARG, e.to_string()))?;

    debug!(printer = %printer, "print_rgba_premul resolved printer");
    ...
    unsafe {
        ...
        debug!(
            ...
            dest_x,
            dest_y,
            "print_rgba_premul: layout computed"
        );
...
pub fn render_and_print_label(
    ...
    debug!(
        template_w = tmpl.width_mm,
        template_h = tmpl.height_mm,
        fields = tmpl.fields.len(),
        target_dpi,
        super_sample_scale,
    ...
    debug!(
        width,
        height, "render_and_print_label: downsampled and threshold applied"
    );
