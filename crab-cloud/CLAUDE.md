...
└── db/
    ...
    ├── sync_store.rs      # 边缘同步数据写入 (归档+信用单+发票, huella 验证)
    ├── audit.rs           # 审计日志
    ├── commands.rs        # 远程命令
    ├── email_verifications.rs # 邮箱验证
    ├── tenant_images.rs   # 租户图片管理
...
- P12: AES-256-GCM 加密存 PG，MasterKey 加密。上传时自动提取 NIF 并同步到 edge store_info
...
**双向同步**:
- Edge → Cloud: `sync_store.rs` 接收 edge 同步数据 (归档订单 + 信用单 + 发票)，写入 normalized 表。发票入库前通过 `InvoiceSync::verify_huella()` 验证 SHA-256 完整性
- Cloud → Edge: `api/store/` Console CRUD 操作后通过 WebSocket 推送 `StoreOp` 到 edge；AEAT 状态变更通过 `UpdateInvoiceAeatStatus` 回推
...
- 发票: store_invoices (Verifactu 发票, AEAT 状态跟踪)
