...
#
# Usage:
#   ./deploy/build-cloud.sh                    # Build only (local)
...
if [ "${1:-}" = "push" ]; then
    ...
    echo "==> Building and pushing crab-cloud (git: $GIT_HASH, platform: linux/arm64)"
    docker buildx build \
        --platform linux/arm64 \
        --provenance=false \
        -f crab-cloud/Dockerfile \
        --build-arg GIT_HASH="$GIT_HASH" \
        -t "$ECR_URI/$REPO_NAME:$IMAGE_TAG" \
        -t "$ECR_URI/$REPO_NAME:$GIT_HASH" \
        --push \
        .

    echo "==> Pushed: $ECR_URI/$REPO_NAME:$IMAGE_TAG"
    echo "==> Pushed: $ECR_URI/$REPO_NAME:$GIT_HASH"
else
    echo "==> Building crab-cloud Docker image (git: $GIT_HASH, platform: linux/arm64)"
    docker buildx build \
        --platform linux/arm64 \
        --provenance=false \
        -f crab-cloud/Dockerfile \
