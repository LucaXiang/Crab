...
- 发票独立于 chain_entry，有自己的 huella 链 (`invoice_counter.last_huella`)
...
**编号体系**：
- `receipt_number`: `{store_number:02}-{YYYYMMDD}-{daily_count:04}` (如 `01-20260306-0001`)，redb 按日计数器
- `credit_note_number`: 与 receipt_number 共享同一 redb daily_count 计数器
- `invoice_number`: `{Serie}-{YYYYMMDD}-{NNNN}`，每终端独立 Serie，`invoice_counter` 表分配

**数据库丢失恢复 (Re-bind Recovery)**：
- Edge 删库后重新绑定时，Cloud 通过 `CatalogSyncData` 附带 `RecoveryState` 恢复计数器
- **订单层**：恢复 redb daily_count + 插入 BREAK chain_entry（诚实标记断链）
- **发票层**：恢复 `invoice_counter`（last_huella + last_number），huella 链不断（Cloud 有完整数据）
- **计数器同步**：Edge 每次同步订单时附带 `CounterState`（daily_count + business_date），Cloud 存储在 `stores` 表
- 安全保证：Edge 需要 catalog 才能服务订单 → catalog 只来自 Cloud → recovery 在 catalog 同步时发生 → 无离线间隙
...
- `edge-server/src/archiving/service.rs` — 归档服务 (哈希链 + chain_entry)
- `edge-server/src/orders/manager/mod.rs` — receipt_number 生成 (redb daily_count)
- `edge-server/src/cloud/ops/provisioning.rs` — FullSync + CatalogSyncData + Recovery State
