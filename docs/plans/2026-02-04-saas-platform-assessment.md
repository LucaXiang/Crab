...
├─────────────┼─────────────┼────────────┤
│ 0-1,000     │ 免费        │ €0         │
│ 1,001-5,000 │ €0.02/单    │ €20-100    │
│ 5,001-20,000│ €0.015/单   │ €75-300    │
│ 20,000+     │ €0.01/单    │ €200+      │
└─────────────┴─────────────┴────────────┘

计费流程:
edge-server ──每小时上报──▶ 云端用量聚合 ──▶ 计费引擎 ──▶ Stripe 扣款
     │                           │
     ▼                           ▼
redb 本地备份              月度结算 + 发票
```
...
| 周次 | 任务 | 交付物 |
|------|------|--------|
| Week 1-2 | 订单计数器 | edge-server 改造完成 |
| Week 3-4 | 用量上报 API | 云端接收服务 |
| Week 5-6 | Stripe 订阅集成 | 支付 Webhook 处理 |
| Week 7-8 | 计费引擎 | 阶梯定价 + 月度结算 |
| Week 9-10 | 客户门户 MVP | 注册/登录/订阅 |
