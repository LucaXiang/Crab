use crab_cert::{
    encrypt, decrypt, sign, verify, CaProfile, CertProfile, CertificateAuthority, KeyType,
};
...
fn test_rsa_crypto_ops() {
    ...
    let mut server_profile = CertProfile::new_server("rsa.local", vec!["rsa.local".into()]);
    server_profile.key_type = KeyType::Rsa2048;
    
    let (cert_pem, key_pem) = ca.issue_cert(&server_profile).expect("Failed to issue RSA cert");
...
fn test_ecdsa_crypto_ops() {
    ...
    let server_profile = CertProfile::new_server("ecdsa.local", vec!["ecdsa.local".into()]);
    // Defaults to P-256
    
    let (cert_pem, key_pem) = ca.issue_cert(&server_profile).expect("Failed to issue ECDSA cert");
