...
export const VoidReasonModal: React.FC<VoidReasonModalProps> = ({
  ...
  const LOSS_REASONS: LossReason[] = ['CUSTOMER_FLED', 'REFUSED_TO_PAY', 'OTHER'];
  ...
  return (
    <div className="fixed inset-0 z-80 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-red-50">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-red-100 rounded-full text-red-600">
              <AlertTriangle size={28} />
            </div>
            <h2 className="text-2xl font-bold text-gray-800">{t('checkout.void.title')}</h2>
          </div>
          <button onClick={onClose} className="p-3 hover:bg-red-100 rounded-full transition-colors text-gray-500">
            <X size={24} />
          </button>
        </div>

        {/* Content - 双栏布局 */}
        <div className="grid grid-cols-2 divide-x divide-gray-100 overflow-y-auto">
          {/* 左栏：金额 + 作废类型 */}
          <div className="p-6 space-y-5">
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              <div className="flex justify-between text-base">
                ...
                <span className="font-semibold text-gray-900">{formatCurrency(paidAmount)}</span>
              </div>
              <div className="flex justify-between text-base mt-2">
                <span className="text-gray-600">{t('checkout.void.unpaid_amount')}</span>
                <span className="font-semibold text-gray-900">{formatCurrency(unpaidAmount)}</span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              >
                <div className={`p-3 rounded-lg shrink-0 ${voidType === 'CANCELLED' ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-500'}`}>
                  <Ban size={24} />
                </div>
                <div>
                  <span className={`text-base font-semibold ${voidType === 'CANCELLED' ? 'text-red-700' : 'text-gray-700'}`}>
              ...
              >
                <div className={`p-3 rounded-lg shrink-0 ${voidType === 'LOSS_SETTLED' ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-500'}`}>
                  <FileWarning size={24} />
                </div>
                <div>
                  <span className={`text-base font-semibold ${voidType === 'LOSS_SETTLED' ? 'text-orange-700' : 'text-gray-700'}`}>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {CANCEL_REASONS.map((key) => (
                    ...
                    >
                      <span className="font-semibold text-base">{t(`checkout.void.cancel_reason.${key}`)}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {LOSS_REASONS.map((key) => (
                    ...
                    >
                      <span className="font-semibold text-base">{t(`checkout.void.loss_reason.${key.toLowerCase()}`)}</span>
                    </button>
                  ))}
                </div>
              </div>
        ...
        {/* Footer */}
        <div className="px-8 py-5 border-t border-gray-100 flex gap-4 bg-gray-50">
          <button
            onClick={onClose}
            className="flex-1 py-4 px-6 rounded-xl font-bold text-lg text-gray-600 hover:bg-gray-200 transition-colors"
          >
          ...
          <button
            ...
            disabled={!canConfirm}
            className={`flex-1 py-4 px-6 rounded-xl font-bold text-lg text-white transition-all shadow-lg ${
              canConfirm
                ? voidType === 'LOSS_SETTLED'
