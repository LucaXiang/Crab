...
import { Package, Printer, Tag, Plus, Trash2, Image as ImageIcon } from 'lucide-react';
...
export const ProductWizard: React.FC<ProductWizardProps> = ({
  ...
  const step1: WizardStep = {
    id: 'basics',
    title: t('settings.product.step_basics') || 'Basics',
    description: t('settings.product.step_basics_desc') || 'Name, Category & Image',
    isValid: !!name.trim() && categoryId !== '',
    component: (
      <div className="space-y-6">
        <div className="flex justify-center">
          <div
            className="w-32 h-32 shrink-0 bg-white rounded-2xl border border-dashed border-gray-300 flex items-center justify-center overflow-hidden cursor-pointer hover:border-primary-400 hover:bg-primary-50/50 transition-all shadow-sm"
          ...
          >
            {image ? (
              ...
              <div className="flex flex-col items-center gap-2 text-slate-400">
                ...
                <span className="text-xs font-medium">{t('common.action.upload')}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <FormField label={t('settings.product.category')} required>
          ...
          {initialCategoryId && categoryId === initialCategoryId && (
             <p className="text-xs text-primary-600 mt-1.5 flex items-center gap-1">
               ...
               {t('settings.product.last_selected_category') || 'Last selected'}
             </p>
          )}
        </FormField>
      </div>
  ...
  const step2: WizardStep = {
    id: 'pricing',
    title: t('settings.product.step_pricing') || 'Pricing',
    description: t('settings.product.step_pricing_desc') || 'Prices & Variants',
    ...
    component: (
      <div className="space-y-4">
          ...
          <div key={idx} className="p-4 bg-slate-50 rounded-xl border border-slate-200 relative group animate-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="grid grid-cols-12 gap-3 items-end">
              <div className="col-span-7">
                <FormField label={t('settings.product.spec_name') || 'Variant Name'}>
                  <input 
                    value={spec.name} 
            ...
            {specs.length > 1 && (
               <div className="mt-3 flex items-center gap-4">
                 <label className="flex items-center gap-2 text-xs text-slate-600 cursor-pointer">
                   ...
                   />
                   {t('settings.product.is_default') || 'Default'}
                 </label>
               </div>
            )}
          </div>
        ...
        >
          ...
          {t('settings.product.add_variant') || 'Add Variant'}
        </button>
      </div>
    ),
  };
  ...
  const step3: WizardStep = {
    id: 'operations',
    title: t('settings.product.step_operations') || 'Operations',
    description: t('settings.product.step_operations_desc') || 'Printing & Tax',
    isValid: true,
    component: (
      <div className="space-y-6">
  ...
  const step4: WizardStep = {
    id: 'org',
    title: t('settings.product.step_org') || 'Organization',
    description: t('settings.product.step_org_desc') || 'Tags & Codes',
    isValid: true,
    component: (
      <div className="space-y-6">
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {tags.length === 0 && (
              <p className="text-sm text-slate-400 italic">{t('settings.product.tags.no_selected') || 'No tags available'}</p>
            )}
          </div>
        </div>
  ...
  return (
    <Wizard
      ...
      title={t('settings.product.create_title') || 'Create Product'}
