...
import { Image as ImageIcon, Tag, Hash, FileText, Layers, ImagePlus, Printer, List, Lock } from 'lucide-react';
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  // Memoize tag lookup map for O(1) access
  ...
  const { systemTagsOnProduct, userTagsOnProduct } = useMemo(() => {
    ...
    for (const id of selectedTagIds) {
      ...
      if (tag) {
        if (tag.is_system) system.push(tag);
        else user.push(tag);
      }
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <FormSection title={t('settings.product.tags.title')} icon={Tag} defaultCollapsed>
        ...
        <div className="min-h-[3.75rem]">
          {systemTagsOnProduct.length === 0 && userTagsOnProduct.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-4 text-gray-400 bg-gray-50 rounded-lg border border-dashed border-gray-200">
      ...
      <TagSelectionModal
        ...
        selectedTagIds={userTagsOnProduct.map((t) => t.id)}
        onChange={(newUserTagIds) => {
          // Preserve system tags, update user tags
          const systemTagIds = systemTagsOnProduct.map((t) => t.id);
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-gray-900">{spec.name || t('settings.product.specification.label.default')}</span>
                    {spec.is_root && (
                ...
                {spec.receipt_name && (
                  <p className="text-xs text-gray-500 mt-1">
                    {t('settings.specification.form.receipt_name')}: {spec.receipt_name}
                  </p>
                )}
              </div>
            ))}
